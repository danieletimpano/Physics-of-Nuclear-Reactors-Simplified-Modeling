...
beta_plus=zeros(1,n);
%Definition of Beta i+1
beta_plus=D;
%Definition of Beta i-1
beta_minus=D;

main_diag= (-(beta_plus + beta_minus)/h^2 - sigma_a)*ones(n,1); %definition of the main diagonal of the matrix
up_diag= beta_plus/h^2*ones(n-1,1); %definition of the upper diagonal of the matrix
low_diag= beta_minus/h^2*ones(n-1,1); %definition of the lower diagonal of the matrix
...
A(1,1)= -beta_plus/h^2 - sigma_a;
b= zeros(n,1);
b(1)= - C/h;
%setting boundary conditions at x=x0
BC1= 1/(2*(h/(4*D) +1)); %zero incoming partial current definition
A(n,n)= (-BC1*h -beta_minus)/h^2 - sigma_a;
...
phi_nit_0= (h*C/(2*D))+ sol(1);
phi_nit_x0= (1/(0.25 + D/h))*(D/h)*sol(end);
