...
%% SOLUTION FOR THE CHOSEN OPTIMISATION PATTERN - LOW LEAKAGE PATTERN
% code for the configuration that allows to:
% - have lower flux at the reflector boundary
% - have higher peak power among the admitted ones
...
D_1t=[D1(1)*ones(n_ass,1); D2(1)*ones(n_ass,1); ...
      D1(1)*ones(n_ass,1); D2(1)*ones(n_ass,1); ...
      D3(1)*ones(n_ass,1); D3(1)*ones(n_ass,1); ...
      D4(1)*ones(n_ass,1)];
D_2t=[D1(2)*ones(n_ass,1); D2(2)*ones(n_ass,1); ...
      D1(2)*ones(n_ass,1); D2(2)*ones(n_ass,1); ...
      D3(2)*ones(n_ass,1); D3(2)*ones(n_ass,1); ...
      D4(2)*ones(n_ass,1)];

%vectors containing removal cross-sections along the mesh

sigma_t1=[ (sigma_a1(1)+ sigma1_s1(2))*ones(n_ass,1); ...
...
sigma_t2=[ (sigma_a1(2)+ sigma1_s2(1))*ones(n_ass,1); ...
...
%boundary condition according to symmetry and zero incoming current
...
%boundary conditions according to symmetry and zero incoming current
...
down_scatter_t= [-sigma1_s1(2)*ones(n_ass,1); -sigma2_s1(2)*ones(n_ass,1);...
                 -sigma1_s1(2)*ones(n_ass,1); -sigma2_s1(2)*ones(n_ass,1); ...
                 -sigma3_s1(2)*ones(n_ass,1); -sigma3_s1(2)*ones(n_ass,1); ...
                 -sigma4_s1(2)*ones(n_ass,1)] ;

%we need to account for the upscattering term from group2 to group1
up_scatter_t= [ -sigma1_s2(1)*ones(n_ass,1);  -sigma2_s2(1)*ones(n_ass,1); ...
                -sigma1_s2(1)*ones(n_ass,1); -sigma2_s2(1)*ones(n_ass,1); ...
                -sigma3_s2(1)*ones(n_ass,1); -sigma3_s2(1)*ones(n_ass,1); ... 
                -sigma4_s2(1)*ones(n_ass,1)] ;

Mt= Mt + diag(down_scatter_t, -nt) + diag(up_scatter_t, nt);

%% Costruction of the F matrix 
ff= [(nusigma_f1(1))*ones(n_ass,1); (nusigma_f2(1))*ones(n_ass,1); ...
     (nusigma_f1(1))*ones(n_ass,1); (nusigma_f2(1))*ones(n_ass,1); ...
     (nusigma_f3(1))*ones(n_ass,1); (nusigma_f3(1))*ones(n_ass,1); ...
     (nusigma_f4(1))*ones(n_ass,1)];
kf1= [(ksigma_f1(1))*ones(n_ass,1); (ksigma_f2(1))*ones(n_ass,1); ...
    (ksigma_f1(1))*ones(n_ass,1); (ksigma_f2(1))*ones(n_ass,1); ...
    (ksigma_f3(1))*ones(n_ass,1); (ksigma_f3(1))*ones(n_ass,1); ...
    (ksigma_f4(1))*ones(n_ass,1)];
ff= [ff; zeros(nt,1)];
tf= [(nusigma_f1(2))*ones(n_ass,1); (nusigma_f2(2))*ones(n_ass,1);...
     (nusigma_f1(2))*ones(n_ass,1); (nusigma_f2(2))*ones(n_ass,1); ...
     (nusigma_f3(2))*ones(n_ass,1); (nusigma_f3(2))*ones(n_ass,1); ...
     (nusigma_f4(2))*ones(n_ass,1)];
kf2= [(ksigma_f1(2))*ones(n_ass,1); (ksigma_f2(2))*ones(n_ass,1);...
      (ksigma_f1(2))*ones(n_ass,1); (ksigma_f2(2))*ones(n_ass,1); ...
      (ksigma_f3(2))*ones(n_ass,1); (ksigma_f3(2))*ones(n_ass,1); ...
...
xline(120, 'r--')
xline(140, 'k--')
title('Numerical solution - Low-leakage pattern')
legend('Flux Numerical Group 1', 'Flux Numerical Group 2', 'Assembly 1', ...
    'Assembly 2', 'Assembly 1', 'Assembly 2', 'Assembly 3', ' Assembly 3', ...
    'Reflector', 'Location','NorthEast', 'FontSize', 8)
grid on
...
P_tot= sum(P); %total power

%relative power computation
P_mean= P_tot/120; %mean power along mesh points
P_rel= P/P_mean; %relative power with respect to the mean
Peak= max(P_rel); %select peak power 

%normalization factor
F_norm= (45e6)/P_tot; %normalisation factor

%normalized flux
phi_norm= F_norm*phi(:,end); %normalised flux

%% PLOTTING - NORMALIZED FLUX
figure(2)
plot(pos_t,phi_norm((1:140),end),pos_t,phi_norm((141:280),end), 'linewidth', 2)
hold on
xline(20, 'g--', 'linewidth', 2)
xline(40, 'b--', 'linewidth', 2)
xline(60, 'g--', 'linewidth', 2)
xline(80, 'b--', 'linewidth', 2)
xline(100, 'r--', 'linewidth', 2)
xline(120, 'r--', 'linewidth', 2)
xline(140, 'k--', 'linewidth', 2)
title('Normalised solution - Low-leakage pattern')
legend('Flux Numerical Group 1', 'Flux Numerical Group 2', 'Assembly 1', ...
    'Assembly 2', 'Assembly 1', 'Assembly 2', 'Assembly 3', ' Assembly 3', ...
    'Reflector', 'Location','NorthEast', 'FontSize', 8)
grid on
%saveas(gcf,'LLP_Flux.png')
%saveas(gcf,'LLP_Flux','epsc')
...
xline(120, 'r--','linewidth', 2)
xline(140, 'k--', 'linewidth', 2)
title('Relative Power - Low-leakage pattern')
legend('Relative Power', 'Assembly 1', ...
    'Assembly 2', 'Assembly 1', 'Assembly 2', 'Assembly 3', ' Assembly 3', ...
    'Reflector', 'Location','NorthEast', 'FontSize', 8)
grid on
%saveas(gcf,'LLP_Pow.png')
%saveas(gcf,'LLP_Pow','epsc')
%% HINT PARAMETERS
